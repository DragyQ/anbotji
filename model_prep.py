...
class Trainer():
    ...
    def execute(self):
        ...
        for epoch in range(epochs):
            avg_mae_personality, avg_mse_personality, avg_mae_iri, avg_mse_iri, precision_emotions, recall_emotions, f1_emotions, train_loss = self.train(train_loader, optimizer)
            print(f"Epoch {epoch+1}: Train Loss: {train_loss:.4f}")
            print(f"Personality -> MAE: {avg_mae_personality:.4f}, MSE: {avg_mse_personality:.4f}")
            print(f"IRI -> MAE: {avg_mae_iri:.4f}, MSE: {avg_mse_iri:.4f}")
            print(f"Emotions -> Precision: {precision_emotions:.4f}, Recall: {recall_emotions:.4f}, F1 Score: {f1_emotions:.4f}")

            avg_mae_personality_dev, avg_mse_personality_dev, avg_mae_iri_dev, avg_mse_iri_dev, precision_emotions_dev, recall_emotions_dev, f1_emotions_dev,dev_loss = self.evaluate(dev_loader)
            ...
            if (avg_mae_personality_dev < last_best_per and avg_mse_personality_dev < last_best_per and avg_mae_iri_dev < last_best_iri and avg_mse_iri_dev < last_best_iri and f1_emotions_dev > last_best_emotions):
              print("Saving model..")
